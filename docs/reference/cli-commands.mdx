...
uv run python analyze.py results.json --full-pipeline --context "Key themes" --model deepseek
uv run python analyze.py results.json --context "Summarise" --local
uv run python analyze.py results.json --context "Summarise" --model deepseek --dry-run
```
