...
Triage uses local Mistral 7B (free, private). Checkpoints save every 10
batches — if interrupted, re-run the same command to resume.

### 6. Deep Analysis (on triaged results)

Start with a lower threshold to avoid missing borderline documents:
